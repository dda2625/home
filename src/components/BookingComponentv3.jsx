    ...
    const fetchBookingData = async () => {
      try {
        ...
        for (const session of BookingApiData.data) {
            if (new Date(session.time_start).toDateString() == new Date().toDateString()  ) {
        ...
        const endDate = new Date(currentDate.getTime() + (6 * 24 * 60 * 60 * 1000)); // Get the date 6 days from now

        for (const session of noneFilterdBookings) {
            const sessionDate = new Date(session.time_start).toDateString();
            if (new Date(session.time_start) >= currentDate && new Date(session.time_start) <= endDate) {
      ...
      <tbody className="h-full">
        ...
        {SessionToday.map((booking, index) => (
          ...
          >
              ...
              <td className="pl-[4px]">{convertZulu(fixNetworkTime(booking.logon_time))}</td>
              <td className="pl-[4px]">-</td>
          </tr>
        ))}
        ...
        {BookingsToday.length > 0 ? "" : <td colSpan={4} className="h-12">No online ATC at the moment</td>}
        {Object.keys(bookingsNotToday).map((date) => ( 
            <>
                <tr className="bg-[#d5dfdf] dark:bg-[#1b3546] w-full font-bold text-black dark:text-white py-4 text-center">
                    <td colSpan={4} className="py-1">{date}</td>
                </tr>
